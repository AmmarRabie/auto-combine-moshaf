...
class SalahSplitter:
    ...
    def _getActualCut(self, length, position):
        '''
            handle appending silence range shifts
        '''
        if not self.append_silence_before_run: return position
        subtraction = min(position, self.initial_window_size)
        position = min(position, length)
        if position > length - self.initial_window_size:
            subtraction += self.initial_window_size - (length - position)
        return position - subtraction
...
def test():
    def _getActualCut(length, position):
        if not True: return position
        subtraction = min(position, 3)
        if position > length - 3:
            subtraction += 3 - (length - position)
        return position - subtraction
    print(_getActualCut(10, 6)) # 3
    print(_getActualCut(10, 7)) # 4
    print(_getActualCut(10, 9)) # 4
